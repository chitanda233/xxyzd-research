...
网站入口为 `docs/index.html`。策划反拆按独立模块并列组织：局内整体链路、怪物与弹幕、三选一与武器进化、章节规划、升级词条、战机与皮肤、局外系统。各模块不是局内整体链路的下级；工具区只承担查表与数据查询。正式报告与导航由 `research-data/tools/planner_reports.py` 统一生成。
