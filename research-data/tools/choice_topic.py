...
<h2 id="entry">1. 成长入口：相似弹窗不能混为一个机制</h2>''')
 ...
 parts.append(refs('C01','C06','C10','C11','C12'))
 ...
 parts.append('''<h3>宝箱内容池如何生成</h3><ol><li>读取当前构筑对应的技能池，提取二星及以上的升级条目，并保留其权重、前置技能与限制条件。</li><li>按当前合法类别和池内权重逐项抽取。被动、武器、特殊升级的基础类别权重为200、300、300；空池与条件筛选会改变实际机会。</li><li>每抽一项处理后续技能和进化依赖，使后续抽取适配更新后的候选状态。</li><li>候选不足时从补位池补足，再整理升级连续性，生成最终1／3／5项结果，整批发放。</li></ol><p>内部16项是候选构建容量，不是实际奖励数量。最终展示顺序会打乱；单个技能的最终概率需要结合当前构筑计算。</p>'''+refs('C17'))
 ...
 for p,label in [('README.md','专题数据说明'),('facts.json','规则与边界'),('chapters.json','71章配置映射'),('pool-weights.json','技能权重'),('evolution-recipes.json','22条配方'),('constants.json','原生常量'),('mainline-chest-pool.json','主线宝箱规则与内容池'),('treasure-chain.json','宝箱完整调用链'),('treasure-schedule.json','416条宝箱事件'),('treasure-chapters.json','142组章节/分支汇总'),('probability-examples.json','概率示例'),('questions.json','缺口与断点'),('sources.json','证据来源'),('manifest.json','哈希清单')]:parts.append(f'<p><a href="{base+p}">{label}</a></p>')
