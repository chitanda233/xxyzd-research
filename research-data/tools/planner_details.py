...
def aircraft():
 ...
 out.append(block('形态与能力如何对应',['Role_Role记录母体与等级阶段字段，Role_Skin的8个形态分别配置品质、解锁资源、实体模型与关闭开关；Role_SkinStar每个形态给五星成本、属性记录和能力节点。跨形态等级与属性继承尚无运行证据。','下表按形态列出配置状态、品质与是否有逐星能力；配置关闭只表示当前静态开关，不说明所有线上渠道。'],['形态','配置ID','品质','关闭标记','逐星能力节点数','解锁所需'],[[lang.get(x['NameId'],x['NameId']),x['Id'],x['Quality'],x['IfClose'],sum(bool(r['FeatureOnStarUp']) for r in star if r['skinId']==x['Id']),str(x['UnlockCost'])] for x in skin],['planner/inputs/Role_Role.json','planner/inputs/Role_Skin.json','planner/inputs/Role_SkinStar.json']))
 rows=[]
 for x in skin:
 ...
 out.append(block('形态资源来自哪里、能换几次',['第一周签到表第2天配置守卫资源1份。飞鹰号公会商品3011要求公会等级7、公会币15,000，limit=1、refresh=7。毒蝎有1条、黑武士和幻影各有2条资源商品；每条商品的永久限购8不能当作该机体跨渠道总上限。','解锁用同名资源1份，后续星级成本按可消费节点累加为1+1+2+3份，总共8份。满星行成本空数组，不应再加一次。以下列的是已核对的形态资源商品；配置不证明线上同时开放或实际重置行为。'],['商品ID','配置说明','价格项','获得资源','限购','刷新字段','解锁条件'],rg,['planner/inputs/Shop_ShopGoods.json','planner/inputs/SignIn_SignIn.json','planner/inputs/Role_SkinStar.json']))
 return ''.join(out)
def systems():
 ...
 out.append(block('从战斗到下一次战斗的可查连接',['结束奖励、首通、累计星级分别有配置入口。局外永久能力包含武器升星、收藏与形态成长；但本专题只抽样了2/110条武器升星、2/496条收藏升星和9/704条商品。以下只能证明样例字段链，不能给全系统产销或玩家每局收入。属性总合成与服务端发奖仍需专门闭合。'],refs=['in-run/facts.json','planner/sources.json','planner/inputs/AirplaneGun_Star.json','planner/inputs/Collection_TreasureStar.json']))
 out.append(block('兑换和抽取：具体商品怎样限制频率',['Shop_Shop的34条记录定义商店用途、商品ID集合与刷新；本专题Shop_ShopGoods快照仅含9条选取商品，因此下表多数商品ID没有随本报告提供价格与奖励。金币商店商品1、2的价格和奖励已逐行核对；体力商店的“递增购买”目前只见note，未核对应阶梯商品。','竞技商品1001出现重要字段冲突：note称门票，price=钻石2×20，但reward=物品1×50；不能把note当作实际门票到账证据。'],['商店ID','配置用途','商品ID（前8个）','刷新字段','限购字段'],[[x['id'],x['note'],','.join(map(str,x['goods'][:8])),x['refresh'],x['limit']] for x in shop[:12]],['planner/inputs/Shop_Shop.json','planner/inputs/Shop_ShopGoods.json','planner/inputs/CrossArena_CrossArenaSeason.json']))
 out.append(block('七种局外抽取箱的配置边界',['以下仅列入口参数；七行BoxRateIDs都为空，不能从本表计算抽取概率。FreeCD的业务时间单位、线上免费资格、关联概率表和服务端掉落需要继续核对。'],['箱ID','钥匙ID','FreeCD原值','单抽成本原值','十抽原值','DayLimit原值'],[[x['ID'],x['Key_ID'],x['FreeCD'],str(x['Single']),x['Ten'],x['DayLimit']] for x in loot],['planner/inputs/Shop_LootBox.json']))
