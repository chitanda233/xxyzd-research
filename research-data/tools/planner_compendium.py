...
def aircraft():
    ...
    out=['<div class="compendium-intro"><h2>机体养成把“新的打法”和“长期收集”绑在一起</h2><p>角色母体、形态、星级、技能节点和获取商品属于不同配置层。母体层有等级阶段字段，形态层有独立星级字段；这些字段本身不能证明切换机体时进度或属性怎样继承。报告把能力文案、配置成本和渠道限购连起来，实际触发和跨机体继承仍需运行核验。</p></div>',process([('母体','角色身份与等级字段'),('形态','品质与专属配置'),('星级','成本、属性、能力'),('商品','资源渠道与限购'),('实战','验证触发与继承')])]
    out.append(section('1 · 从母体到五星能力经过什么配置',['Role_Role保存母体身份和等级阶段字段；Role_Skin给8种形态的品质、解锁资源、模型与静态关闭标记；Role_SkinStar再按每一星配置成本、属性和FeatureOnStarUp技能。技能描述需要回连Skill_Main与语言表。','这能证明数据模型分层，尚不能证明角色等级、战力或已投入资源在切换形态时怎样保留。静态存在的形态不等于线上当期可获得：是否关闭、商店/活动是否开放与运行渠道需要分别确认。'],['层','数据表','回答的问题'],[['角色母体','Role_Role','身份与等级阶段字段；跨形态继承待核'],['形态','Role_Skin','是哪架机体、品质、模型、解锁资源和关闭标记'],['五星节点','Role_SkinStar','每次升星消耗、属性与能力节点'],['技能文案','Skill_Main + Language','玩家可读的条件与效果'],['获取渠道','Shop_ShopGoods / SignIn','资源从哪里来、受什么限购']],['planner/inputs/Role_Role.json','planner/inputs/Role_Skin.json','planner/inputs/Role_SkinStar.json','planner/inputs/Skill_Main.json']))
    ...
    out.append(section('4 · 不同渠道是怎样把机体接到外围玩法',['第一周签到表第2天配置守卫资源1份；飞鹰号商品3011配置公会等级7、15,000公会币、refresh=7、limit=1。毒蝎商品1302配置永久限购8；黑武士与幻影各有两条兑换商品，不能把每条上限8误认为同一机体总共只能换8份。','这些是客户端商品配置，不保证相应活动同时上线。真实完成周期还要核对商店开放、货币产出、限购重置与服务器发奖。'],['渠道','已核对门槛或频率','连接的设计目标'],[['签到','第一周第2天守卫资源1份','早期接触形态解锁'],['公会商品3011','等级7、15,000公会币、refresh=7、limit=1','长期参与与周节奏'],['活动商品','毒蝎1条、黑武士2条、幻影2条；各商品limit=8','多渠道供应；总获取上限未证']],['planner/inputs/SignIn_SignIn.json','planner/inputs/Shop_ShopGoods.json','planner/inputs/Role_SkinStar.json']))
...
def systems():
    ...
    out=['<div class="compendium-intro"><h2>局外系统要看资源如何从一局进入下一局</h2><p>目前能核实的是部分客户端配置链，并非完整产销表：武器升星只抽样2/110行，收藏只抽样2/496行，商品只保留9/704行。以下逐一标明已闭合的字段关系与尚待核实的到账、消耗和服务端状态。</p></div>',process([('对局','完成与评价'),('结算','直接/待领取奖励'),('资源','抽取与兑换'),('养成','武器、收藏、机体'),('再挑战','章节/竞技/公会')])]
    ...
    out.append(section('2 · 三条永久能力线各消耗什么',['武器升星样例2010101消耗武器材料20101×1，记录物理伤害与暴击数值及两个技能节点；收藏样例6300100消耗材料601×100，记录攻击40、最大生命160及等级限制1；机体形态另有逐星资源、攻击属性和机制技能。','这里的武器与收藏快照分别只包含前2条，而原表有110和496条；不能据此宣称已核完整个系统的成本和收益。三种属性的实际作用对象、合成顺序及装备状态仍需运行逻辑，不能直接相加成战力。'],['系统','已核对样例','对下一局的候选作用'],[['武器升星','2010101：20101×1；物理伤害、暴击与技能节点','属性及关联技能，装配与合成待核'],['收藏','6300100：601×100；攻击40、最大生命160、等级限制1','长期属性，生效与叠加待核'],['机体形态','8架×5星；消耗同名资源并关联能力文案','属性与条件技能，触发待核']],['planner/inputs/AirplaneGun_Star.json','planner/inputs/Collection_TreasureStar.json','planner/inputs/Role_SkinStar.json']))
    out.append(section('3 · 商店要把“店”和“商品”两张表接起来',['Shop_Shop的34条完整记录给出店铺与商品ID集合；当前打包的Shop_ShopGoods只选取9/704条，不能把店铺的商品清单误认为价格和奖励都已核完。已闭合的样例是商店1挂商品1、2：分别配置20钻换2000金币、100钻换10000金币。公会商店21挂商品3011：配置15000公会币换飞鹰号资源1份，要求公会等级7，单商品limit=1、refresh=7。','商品1001尤其需要继续反编译/运行核验：note写“20钻石换竞技场门票”，price为钻石2×20，但reward和showReward却都是物品1×50。报告不能把这行直接当成已证明的门票到账；赛季表的TicketGoodsId=1001只能证明引用关系。'],['已核对链','配置字段','尚未证明'],[['商店1→商品1/2','2×20→1×2000；2×100→1×10000','线上定价与购买结算'],['商店21→商品3011','128×15000→10105×1；GuildLv=7','实际开放和重置时点'],['赛季→商品1001','TicketGoodsId=1001；note称门票，reward=1×50','物品116门票是否通过别的逻辑到账']],['planner/inputs/Shop_Shop.json','planner/inputs/Shop_ShopGoods.json','planner/inputs/CrossArena_CrossArenaSeason.json']))
    out.append(section('4 · 局外抽取和局内接触宝箱是两种经济',['局外Shop_LootBox有7条入口记录，给钥匙、FreeCD、单/十抽成本、DayLimit等参数；7行的BoxRateIDs全为空。只凭这张表不能算抽取概率或证明具体掉落内容，还必须继续追关联的规则/概率表及运行逻辑。局内接触宝箱则来自章节脚本，在战斗中生成1／3／5项词条并批量学习。','FreeCD数值不能直接解释为天数，DayLimit=999也不能推断实际每日可抽999次。服务端资格、货币扣除和奖励到账均未闭合。'],['局外抽取箱','钥匙ID','FreeCD原值','单抽成本原值','十抽原值','DayLimit原值'],[[x['ID'],x['Key_ID'],x['FreeCD'],str(x['Single']),x['Ten'],x['DayLimit']] for x in loot],['planner/inputs/Shop_LootBox.json','topics/choices-box-evolution/mainline-chest-pool.json']))
