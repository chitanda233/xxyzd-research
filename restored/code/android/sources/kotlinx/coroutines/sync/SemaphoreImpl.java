...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1925constructorimpl;
        boolean z;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1925constructorimpl = SegmentOrClosed.m1925constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1925constructorimpl = SegmentOrClosed.m1925constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1930isClosedimpl(objM1925constructorimpl)) {
                Segment segmentM1928getSegmentimpl = SegmentOrClosed.m1928getSegmentimpl(objM1925constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() < segmentM1928getSegmentimpl.getId()) {
                        if (!segmentM1928getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                            z = false;
                            break;
                        }
                        if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM1928getSegmentimpl)) {
                            if (segment.decPointers$kotlinx_coroutines_core()) {
                                segment.remove();
                            }
                        } else if (segmentM1928getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                            segmentM1928getSegmentimpl.remove();
                        }
                    }
                    z = true;
                    break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1928getSegmentimpl(objM1925constructorimpl);
        int i = (int) (andIncrement % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i, (Object) null, cont)) {
            cont.invokeOnCancellation(new CancelSemaphoreAcquisitionHandler(semaphoreSegment3, i));
            return true;
        }
        if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (segment.getId() < segmentM1928getSegmentimpl.getId()) {
                    ...
                    if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM1928getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM1928getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
