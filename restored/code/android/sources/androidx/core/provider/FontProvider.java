...
class FontProvider {
    ...
    static FontsContractCompat.FontInfo[] query(Context context, FontRequest fontRequest, String str, CancellationSignal cancellationSignal) {
        ...
        Uri uriBuild = new Uri.Builder().scheme(IAdInterListener.AdProdType.PRODUCT_CONTENT).authority(str).build();
        Uri uriBuild2 = new Uri.Builder().scheme(IAdInterListener.AdProdType.PRODUCT_CONTENT).authority(str).appendPath(n.z).build();
        ...
        try {
            ...
            if (cursorQuery != null && cursorQuery.getCount() > 0) {
                ...
                while (cursorQuery.moveToNext()) {
                    ...
                    int i4 = columnIndex5 != -1 ? cursorQuery.getInt(columnIndex5) : MediaPlayer.MEDIA_PLAYER_OPTION_USE_CODEC_POOL;
                    if (columnIndex6 == -1 || cursorQuery.getInt(columnIndex6) != z) {
                        z = false;
                    }
                    arrayList2.add(FontsContractCompat.FontInfo.create(uriWithAppendedId, i3, i4, z, i2));
                    z = true;
                    i = 0;
                }
