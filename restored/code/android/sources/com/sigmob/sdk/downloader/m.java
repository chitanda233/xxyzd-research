...
public class m {
    ...
    final c c = new c() { // from class: com.sigmob.sdk.downloader.m.1
        ...
        public void a(f task, int blockIndex, int responseCode, Map<String, List<String>> responseHeaderFields) {
            c[] cVarArrB = m.b(task, m.this.f3465a);
            if (cVarArrB == null) {
                return;
            }
            for (c cVar : cVarArrB) {
    ...
    final SparseArray<ArrayList<c>> f3465a = new SparseArray<>();
    ...
    public synchronized void a(c listener) {
        ...
        for (int i = 0; i < size; i++) {
            ArrayList<c> arrayListValueAt = this.f3465a.valueAt(i);
            if (arrayListValueAt != null) {
                arrayListValueAt.remove(listener);
                if (arrayListValueAt.isEmpty()) {
                    arrayList.add(Integer.valueOf(this.f3465a.keyAt(i)));
                }
            }
        }
        Iterator it = arrayList.iterator();
        while (it.hasNext()) {
            this.f3465a.remove(((Integer) it.next()).intValue());
        }
    }
    ...
    public synchronized boolean a(f task, c listener) {
        int iC = task.c();
        ArrayList<c> arrayList = this.f3465a.get(iC);
        if (arrayList == null) {
            return false;
        }
        boolean zRemove = arrayList.remove(listener);
        if (arrayList.isEmpty()) {
            this.f3465a.remove(iC);
        }
        return zRemove;
    }
