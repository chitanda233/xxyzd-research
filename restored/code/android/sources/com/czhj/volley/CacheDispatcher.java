...
public class CacheDispatcher extends Thread {
    ...
    private static final boolean f2308a = VolleyLog.DEBUG;
    ...
    private static class WaitingRequestManager implements Request.NetworkRequestCompleteListener {
        ...
        /* JADX INFO: renamed from: a, reason: collision with root package name */
        private final Map<String, List<Request<?>>> f2310a = new HashMap();
        ...
        public synchronized boolean a(Request<?> request) {
            ...
            if (!this.f2310a.containsKey(cacheKey)) {
                this.f2310a.put(cacheKey, null);
            ...
            List<Request<?>> arrayList = this.f2310a.get(cacheKey);
            if (arrayList == null) {
                arrayList = new ArrayList<>();
            }
            request.addMarker("waiting-for-response");
            arrayList.add(request);
            this.f2310a.put(cacheKey, arrayList);
            if (VolleyLog.DEBUG) {
        ...
        public synchronized void onNoUsableResponseReceived(Request<?> request) {
            String cacheKey = request.getCacheKey();
            List<Request<?>> listRemove = this.f2310a.remove(cacheKey);
            if (listRemove != null && !listRemove.isEmpty()) {
                ...
                this.f2310a.put(cacheKey, listRemove);
        ...
        public void onResponseReceived(Request<?> request, Response<?> response) {
            ...
            String cacheKey = request.getCacheKey();
            synchronized (this) {
                listRemove = this.f2310a.remove(cacheKey);
            }
            if (listRemove != null) {
                if (VolleyLog.DEBUG) {
