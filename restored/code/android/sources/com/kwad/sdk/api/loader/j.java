...
final class j {
    ...
    static {
        aIj = EH() ? "dynamic_apk" : "dynamic.apk";
    }
    ...
    private static File i(File file) {
        ...
        if (!(file.exists() && file.isDirectory()) && com.kwad.sdk.api.a.oV.booleanValue()) {
    ...
    static File v(Context context, String str) {
        return new File(bq(context), "dynamic-" + System.currentTimeMillis() + "-" + str + ".apk");
    }
    ...
    static File x(Context context, String str) {
        return i(new File(bq(context), "apk-" + str));
    }

    static String y(Context context, String str) {
        return new File(w(context, str), aIj).getPath();
    }
            ...
            public final void doTask() {
                try {
                    ...
                    for (File file : fileArrListFiles) {
                        if (i.M(str, file.getName().substring(file.getName().indexOf("-") + 1))) {
                            j.h(file);
                        }
                    }
                } catch (Exception unused) {
