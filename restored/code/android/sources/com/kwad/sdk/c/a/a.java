...
public final class a {
    ...
    public static int getStatusBarHeight(Context context) {
        ...
        if (identifier > 0) {
            aJd = context.getResources().getDimensionPixelSize(identifier);
        } else {
            try {
                aJd = context.getResources().getDimensionPixelSize(((Integer) ab.getField("com.android.internal.R$dimen", "status_bar_height")).intValue());
            } catch (Throwable th) {
                th.printStackTrace();
            }
        }
    ...
    public static Point bB(Context context) {
        Point pointBC = bC(context);
        Point pointBD = bD(context);
        if (a(pointBC, pointBD)) {
            return new Point(pointBD.x - pointBC.x, pointBC.y);
        }
        if (b(pointBC, pointBD)) {
            return new Point(pointBC.x, pointBD.y - pointBC.y);
        }
        return new Point();
    }

    private static Point bC(Context context) {
    ...
    public static Point bD(Context context) {
        Display defaultDisplay = ((WindowManager) context.getSystemService("window")).getDefaultDisplay();
        Point point = new Point();
    ...
    public static boolean Ii() {
        ...
        if (Math.abs(jUptimeMillis - alB) < 500) {
            alB = jUptimeMillis;
            return true;
        }
        alB = jUptimeMillis;
